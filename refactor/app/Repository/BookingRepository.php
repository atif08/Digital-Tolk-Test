...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory( $user_id, Request $request ) {
        ...
        if ( $cuser && $cuser->typeIs( 'customer' ) ) {
            $jobs = $cuser->jobs()
                          ->with( 'user.userMeta',
                                  'user.average',
                                  'translatorJobRel.user.average',
        ...
        elseif ( $cuser && $cuser->typeIs( 'translator' ) ) {
    ...
    public function store( $user, $data ) {
        ...
        if ( $user->user_type == config( 'digital_tolk.customer_role_id' ) ) {
            $cuser = $user;
    ...
    public function storeJobEmail( $data ) {
        ...
        if ( isset( $data[ 'address' ] ) ) {
            ...
            $job->instructions = ( $data[ 'instructions' ] != '' ) ? $data[ 'instructions' ] :
    ...
    public function sendNotificationTranslator( $job, $exclude_user_id, $data = [] ) {
        ...
        $this->sendPushNotificationToSpecificUsers(
            $translator_array, $job->id, $data, $msg_text,
            false
        );
    ...
    public function sendSMSNotificationToTranslator( $job ) {
        ...
        foreach( $translators as $translator ) {
            ...
            $status = SendSMSHelper::send( config( 'digital_tolk.sms_number' ), $translator->mobile, $message );
    ...
    public function getPotentialTranslators( Job $job ) {
        ...
        return User::getPotentialUsers( $translator_type,
                                        $joblanguage,
                                        $gender,
                                        $translator_level,
                                        $translatorsId );
    ...
    public function updateJob( $id, $data, $cuser ): array {
        ...
        $this->logger->info( 'USER #' . $cuser->id . '(' . $cuser->name . ')' . ' has been updated booking <a class="openjob" href="/admin/jobs/' . $id . '">#' . $id . '</a> with data:  ',
                             $log_data );
    ...
    private function sendNotificationChangePending( $user, $job, $language, $due, $duration ) {
        ...
        if ( $this->isNeedToSendPush( $user->id ) ) {
            $users_array = [ $user ];
            $this->sendPushNotificationToSpecificUsers( $users_array, $job->id, $data,
                                                        $msg_text,
                                                        $this->isNeedToDelayPush( $user->id ) );
        }
    }
        ...
        if ( $cuser->typeIs( 'customer' ) ) {
            ...
            if ( $job->due->diffInHours( Carbon::now() ) > 24 ) {
                ...
                $this->sendNotificationTranslator( $job, $data,
                                                   $translator->id );   // send Push all sutiable translators
                $response[ 'status' ] = 'success';
            }
            else {
                $response[ 'status' ]  = 'fail';
    ...
    public function endJob( $post_data ) {
        ...
        Event::fire( new SessionEnded( $job, ( $post_data[ 'user_id' ] == $job->user_id ) ? $tr->user_id :
            $job->user_id ) );
    ...
    public function getAll( Request $request, $limit = null ) {
        ...
        if ( $cuser && $cuser->user_type == config( 'digital_tolk.superadmin_role_id' ) ) {
    ...
    public function alerts() {
        ...
        if ( $cuser && $cuser->typeIs( 'superadmin' ) ) {
            if ( isset( $requestdata[ 'lang' ] ) && $requestdata[ 'lang' ] != '' ) {
    ...
    public function bookingExpireNoAccepted() {
        ...
        if ( $cuser && ( $cuser->typeIs( 'superadmin' ) || $cuser->typeIs( 'admin' ) ) ) {
            $allJobs = DB::table( 'jobs' )
                         ->join( 'languages', 'jobs.from_language_id', '=', 'languages.id' )
    ...
    public function reopen( $request ) {
        ...
        //        $this->logger->info('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
